...
class LList implements List {
    ...
    //Method to search linked list and return boolean based on whether the list contains the Object
    boolean contains(Object item) {
        boolean contains = false;  //Set it to false
        moveToStart(); //Curr could be anywhere
        for (moveToStart(); !isAtEnd(); next()) {
            if (getValue() == item) { //If the value is equal to the parameter, return it
                contains = true;
                break; // Have to have this or it will always return false
            }
        }
        return contains;
    }

    //Method to return the element at a desired index
    Object get(int index){
        moveToPos(index); //Go to desired location
        Object value = getValue(); // Get the value and return it
        return value;
    }
